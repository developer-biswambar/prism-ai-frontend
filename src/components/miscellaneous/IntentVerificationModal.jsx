    ...
    const DataFlowDiagram = ({steps}) => {
        ...
        const getStepColor = (step) => {
            switch (step.type) {
                ...
                case 'grouping':
                    return 'bg-cyan-50 border border-cyan-200';
                case 'aggregation':
            ...
            <div className="space-y-3">
                {steps.map((step, index) => (
                    <div key={index} className="flex items-start space-x-4">
                        {/* Step Number */}
                        <div
                            className="flex-shrink-0 w-6 h-6 rounded-full bg-gray-200 text-gray-600 text-xs flex items-center justify-center font-medium mt-1">
                            {index + 1}
                        </div>
                        ...
                        <div className={`flex-1 p-4 rounded-lg ${getStepColor(step)}`}>
                            <div className="flex items-start space-x-3">
                                <div
                                    className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${getIconColor(step)}`}>
                                    {getStepIcon(step)}
                                </div>
                                    ...
                                    {step.categories && (
                                        <div className="flex flex-wrap gap-1 mt-2">
                                            {step.categories.map((category, i) => (
                                                <span key={i}
                                                      className="text-xs bg-white rounded-full px-2 py-1 text-gray-600">
                          {category}
                        </span>
                                            ))}
                                        </div>
                                    ...
                                    {step.operations && (
                                        <div className="flex flex-wrap gap-1 mt-2">
                                            {step.operations.map((op, i) => (
                                                <span key={i}
                                                      className="text-xs bg-white rounded px-2 py-1 font-mono text-gray-700">
                          {op}
                        </span>
                                            ))}
                                        </div>
                    ...
                    {intentData?.generated_sql && (
                        <div>
                            ...
                            >
                                ...
                                <ArrowDown size={16}
                                           className={`transform transition-transform ${showSqlQuery ? 'rotate-180' : ''}`}/>
                            </button>
